    ...
    for badge in BADGES:
        ...
        for need in badge["needs"]:
            if need not in keys:
                ...
                cause = "spread"
                # Worded without "undefined", which the smoke test rightly treats as a rendering bug
                # wherever a reader can see it.
                blocked = (f"{need} has zero spread {peers}, so no departure from normal can be "
                           f"expressed in standard deviations")
            if blocked:
                break
        if blocked:
